...
export const CheatingLogProvider = ({children}) =>{
    ...
    const updateCheatingLog = (newLog) =>{
        setCheatingLog((prev)=>{
            // Ensure all count fields are number and value default value
            const updatedLog ={
                ...prev,
                ...newLog,
    ...
    return( <CheatingLogContext.Provider value ={{ cheatingLog, updateCheatingLog, resetCheatingLog}}>
        {children}
...
export const useCheatingLog = () =>{
    ...
    if(!context){
        throw new Error('useCheatingLOg must be used with a CheatingLogProvider');
    }
    return context;
};
